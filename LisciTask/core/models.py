...
class LearningActivity(models.Model):
    ...
    course = models.ForeignKey(
        Course, related_name="learning_activities", on_delete=models.CASCADE
    )
...
class UserData(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="user_data", on_delete=models.CASCADE
    )
    learn_unit = deprecate_field(
        models.ForeignKey(
            Course, related_name="course_user_data", on_delete=models.CASCADE, null=True
        )
    )
    ...
    status = models.CharField(
        max_length=100, default="", blank=True, choices=STATUS.choices
    )
    ...
    def mark_course_as_completed(self):
        progress = UserData.objects.filter(
            learning_activity_id=self.learning_activity_id, user_id=self.user.id
        ).last()
