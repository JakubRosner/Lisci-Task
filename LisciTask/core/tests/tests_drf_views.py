...
class LearningActivityViewSetCRUDTest(TestCase):
    ...
    def test_create_learning_activity(self):
        ...
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, 201)
        self.assertEqual(LearningActivity.objects.count(), 2)  # Assuming there's already one learning activity created

    def test_retrieve_learning_activity(self):
        url = reverse('learningactivity-detail', kwargs={'pk': self.learning_activity.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_learning_activity(self):
        ...
        response = self.client.patch(url, data, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(LearningActivity.objects.get(pk=self.learning_activity.pk).name, 'Updated Activity')
...
class UserDataViewSetCRUDTest(TestCase):
    ...
    def test_create_user_data(self):
        ...
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, 201)
        self.assertEqual(UserData.objects.count(), 2)  # Assuming there's already one user data created
